...
app.post('/fix', upload.single('video'), (req, res) => {
  ...
  const mode    = req.query.mode || 'standard'; // standard | podcast
  const job = {
...
async function processVideo(job, opts) {
  ...
  try {
    ...
    //
    // ── Podcast mode: ultra-aggressive freeze/black frame removal ──
    // Uses freezedetect to find black/frozen segments, then cuts them
    // and uses minterpolate to synthesise smooth motion across the gaps.
    // hi/lo thresholds set very low to catch even subtle freezes.
    const vf = isPodcast ? [
      // Stage 1: Remove duplicate AND near-duplicate frames
      // hi=64*8 = very sensitive (catches subtle freezes, not just full freezes)
      // frac=0.1 = only 10% of blocks need to match for frame to be flagged
      `mpdecimate=max=0:hi=64*8:lo=64*4:frac=0.1`,
      // Stage 2: Re-time — fills gaps left by removed frames
      `setpts=N/FRAME_RATE/TB`,
      // Stage 3: Motion-compensated interpolation
      // fps=${outFps}: target frame rate
      // mi_mode=mci: motion compensated (best quality, synthesises real frames)
      // mc_mode=aobmc: adaptive overlapped block (handles face/body movement)
      // me_mode=bidir: bidirectional search (looks forward AND backward)
      // vsbmc=1: variable block size (handles fine detail around edges)
      // scd=fdiff: scene change detection via frame diff (no blending across cuts)
      `minterpolate=fps=${outFps}:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1:scd=fdiff`,
      // Stage 4: Denoise — remote video is often heavily compressed
      `hqdn3d=luma_spatial=3:chroma_spatial=2:luma_tmp=4:chroma_tmp=3`,
      // Stage 5: Sharpen after denoise to restore edge detail
      `unsharp=luma_msize_x=5:luma_msize_y=5:luma_amount=2.0:chroma_msize_x=3:chroma_msize_y=3:chroma_amount=0.8`,
      // Stage 6: Upscale if requested
      ...(scaleW !== srcW ? [
